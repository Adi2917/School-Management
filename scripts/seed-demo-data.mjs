...
const schools = await Promise.all(schoolSeeds.map(async ([schoolCode, schoolName, adminName, city], index) => protectCredentials("schools", {
  ...
  email: `adhyetaclasses1729+admin${index + 1}@gmail.com`,
  admin_email: `adhyetaclasses1729+admin${index + 1}@gmail.com`,
    ...
    for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex += 1) {
      for (let roll = 1; roll <= 5; roll += 1) {
        ...
        studentSeeds.push({
          ...
          email: `adhyetaclasses1729+student${String(serial).padStart(4, "0")}@gmail.com`,
