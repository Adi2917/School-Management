...
type StudentForm = { name: string; father_name: string; number: string; school_code: string; class: string; section: string; roll: string; pin: string; address: string };
const emptySchool: SchoolForm = { school_name: '', admin_name: '', email: '', phone: '', school_code: '', admin_pin: '', location: '' };
const emptyStudent: StudentForm = { name: '', father_name: '', number: '', school_code: '', class: '', section: '', roll: '', pin: '', address: '' };
...
export function RegistrationScreen({ role }: { role: 'school' | 'student' }) {
  ...
  const upload = async () => {
    if (!asset) return '';
    return (await api.uploadFile(asset.uri, asset.fileName || `${role}-photo.jpg`, asset.mimeType || 'image/jpeg')).url;
  };
  const submitSchool = async () => {
    ...
    await api.createRecord<School>('schools', { ...value, admin_email: value.email.trim().toLowerCase(), email: value.email.trim().toLowerCase(), school_name: value.school_name.trim(), admin_name: value.admin_name.trim(), location: value.location.trim(), school_logo });
  ...
  const submitStudent = async () => {
    ...
    if ((await api.getStudents(value.school_code)).some(item => String(item.number) === value.number)) throw new Error('A student with this phone number is already registered in this school.');
    const photo_url = await upload();
    await api.createRecord<Student>('students', { ...value, name: value.name.trim(), father_name: value.father_name.trim(), roll: value.roll.trim(), address: value.address.trim(), school_name: matched.school_name, school_logo: matched.school_logo || '', photo_url });
