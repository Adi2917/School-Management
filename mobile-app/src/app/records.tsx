...
export default function RecordsScreen() {
  ...
  const [feeDrafts, setFeeDrafts] = useState<Record<string, { due: string; status: string }>>({});
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setStudent(target);
      setRecords(data);
      if (kind === 'fees') setFeeDrafts(Object.fromEntries((data as Fee[]).map(fee => [fee.id || `${fee.fee_type || 'monthly'}-${fee.month || fee.exam_fee_id}`, { due: String(fee.due_amount ?? 0), status: fee.status || 'Pending' }])));
      setExamTypes(exams);
  ...
  const updateFee = async (fee: Fee, status: string) => {
    ...
    try {
      ...
      await api.updateRecord('fees', fee.id, { status, due_amount: due, paid_at: status === 'Paid' ? new Date().toISOString() : '' });
      await load();
