...
const paidFor=(fee:Fee)=>fee.paid_amount!==undefined?Number(fee.paid_amount):fee.status==='Paid'?Math.max(0,Number(fee.amount||0)+Number(fee.carried_due||0)-Number(fee.due_amount||0)):0;
...
export default function RecordsScreen() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setRecords(data);
      if (kind === 'fees') { const fees=data as Fee[]; setFeeDrafts(Object.fromEntries(fees.map(fee => [fee.id || `${fee.fee_type || 'monthly'}-${fee.month || fee.exam_fee_id}`, { due: String(paidFor(fee)), status: fee.status || 'Pending', duesPaid:false }]))); setSelectedExamFee(current=>current || fees.find(fee=>fee.fee_type==='exam')?.exam_fee_id || ''); }
      setExamTypes(exams);
  ...
  useFocusEffect(useCallback(() => {
    void load();
    // Never replace an admin's in-progress form draft with a background refresh.
    if (admin) return undefined;
    const live=setInterval(()=>void load(true),3000);
  ...
  const ensureFees = async () => {
    ...
    try {
      ...
      setRecords(synced);
      setFeeDrafts(Object.fromEntries(synced.map(fee => [fee.id || `${fee.fee_type || 'monthly'}-${fee.month || fee.exam_fee_id}`, { due: String(paidFor(fee)), status: fee.status || 'Pending', duesPaid:false }])));
  ...
  const updateFee = async (fee: Fee, status: string) => {
    ...
    try {
      const key = fee.id;
      const total=Number(fee.amount||0)+Number(fee.carried_due||0);
      const paidAmount=Math.min(total,Math.max(0,Number(feeDrafts[key]?.due||0)));
      const balance=Math.max(0,total-paidAmount);
      await api.updateRecord('fees', fee.id, { status,paid_amount:paidAmount,due_amount:balance,dues_paid:balance===0,paid_at:status==='Paid'&&paidAmount>0?new Date().toISOString():'' });
  ...
  const removeSubject = (name: string) => {
    setSubjects(current => current.filter(item => item !== name));
    setMarks(current => { const next={...current}; delete next[name]; return next; });
  };

  const deleteResultGroup = (name: string, rows: Result[]) => Alert.alert(
    'Delete complete result?',
    `${name} and all its subject marks will be permanently deleted.`,
    [{text:'Cancel',style:'cancel'},{text:'Delete result',style:'destructive',onPress:async()=>{
      setSaving(true);
      try {
        await Promise.all(rows.filter(row=>row.id).map(row=>api.deleteRecord('results',row.id!)));
        await load();
      } catch(error) {
        Alert.alert('Delete failed',error instanceof Error?error.message:'Please retry.');
      } finally { setSaving(false); }
    }}],
  );
  ...
  return <View style={styles.page}>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="always" keyboardDismissMode="on-drag" automaticallyAdjustKeyboardInsets nestedScrollEnabled>
      ...
      {busy ? <RecordSkeleton /> : kind === 'fees' ? <>
        ...
        <View style={styles.cards}>{visibleFeeRecords.map(fee => {
          const key=fee.id||fee.month,base=Number(fee.amount||0),carry=Number(fee.carried_due||0),total=base+carry;
          const storedPaid=fee.paid_amount!==undefined?Number(fee.paid_amount):fee.status==='Paid'?Math.max(0,total-Number(fee.due_amount||0)):0;
          const draft=feeDrafts[key]||{due:String(storedPaid),status:fee.status,duesPaid:false};
          const paid=admin?Math.min(total,Math.max(0,Number(draft.due||0))):storedPaid,balance=Math.max(0,total-paid);
          return <View key={key} style={styles.feeCard}><View style={styles.feeHead}><View style={styles.flex}><Text style={styles.recordTitle}>{fee.title||fee.month}</Text><Text style={styles.recordCopy}>{fee.fee_type==='exam'?'Exam fee':'Monthly fee'}{fee.paid_at?` · Paid on ${new Date(fee.paid_at).toLocaleDateString()}`:''}</Text></View><Text style={styles.amount}>₹{base.toLocaleString('en-IN')}</Text></View><View style={feeStyles.calculation}><Text>{fee.fee_type==='exam'?'Exam fee':'Monthly fee'} <Text>₹{base.toLocaleString('en-IN')}</Text></Text><Text>Previous dues <Text>₹{carry.toLocaleString('en-IN')}</Text></Text><Text style={feeStyles.totalLine}>Total payable <Text>₹{total.toLocaleString('en-IN')}</Text></Text>{admin?<View style={styles.dueRow}><Text style={styles.dueLabel}>Paid amount</Text><TextInput style={styles.dueInput} value={draft.due} onChangeText={value=>setFeeDrafts(current=>({...current,[key]:{...draft,due:value.replace(/\D/g,'')}}))} keyboardType="number-pad"/></View>:null}<Text style={feeStyles.paidLine}>Total paid <Text>₹{paid.toLocaleString('en-IN')}</Text></Text><Text style={styles.dueLabel}>Balance due ₹{balance.toLocaleString('en-IN')}</Text></View>{admin?<View style={styles.statusOptions}>{['Pending','Paid'].map(status=><Pressable key={status} disabled={saving} onPress={()=>updateFee(fee,status)} style={[styles.statusChoice,(draft.status||fee.status)===status&&styles.statusChoiceActive,status==='Paid'&&(draft.status||fee.status)===status&&styles.paid]}><Text style={styles.statusText}>{status}</Text></Pressable>)}</View>:<View style={[styles.status,fee.status==='Paid'&&styles.paid]}><Text style={styles.statusText}>{fee.status}</Text></View>}</View>})}</View>
      </> : kind === 'notices' ? <>
