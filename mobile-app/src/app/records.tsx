...
export default function RecordsScreen() {
  ...
  const updateFee = async (fee: Fee, status: string) => {
    ...
    try {
      ...
      await api.updateRecord('fees', fee.id, { status, due_amount: due, dues_paid: duesPaid, paid_at: status === 'Paid' ? new Date().toISOString() : '' });
      if ((fee.fee_type || 'monthly') === 'monthly') { const monthly=(records as Fee[]).filter(item=>(item.fee_type||'monthly')==='monthly').sort((a,b)=>MONTHS.indexOf(a.month)-MONTHS.indexOf(b.month)); const next=monthly[monthly.findIndex(item=>item.id===fee.id)+1]; if(next?.id) await api.updateRecord('fees',next.id,{due_amount:duesPaid?0:due}); }
      await load();
  ...
  const toggleDuesPaid = async (fee: Fee, due: number, checked: boolean) => {
    ...
    try { await api.updateRecord('fees',fee.id,{dues_paid:checked,due_amount:due}); const monthly=(records as Fee[]).filter(item=>(item.fee_type||'monthly')==='monthly').sort((a,b)=>MONTHS.indexOf(a.month)-MONTHS.indexOf(b.month)); const next=monthly[monthly.findIndex(item=>item.id===fee.id)+1]; if(next?.id)await api.updateRecord('fees',next.id,{due_amount:checked?0:due}); await load(); }
