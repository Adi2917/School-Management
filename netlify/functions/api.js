...
export default async function handler(request) {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      if (collection === "students") { for (const item of rawItems) { const email=String(item.email||"").trim().toLowerCase(); const number=String(item.number||"").trim(); const slot={school_code:String(item.school_code||"").trim(),class:String(item.class||"").trim(),section:String(item.section||"").trim().toUpperCase(),roll:String(item.roll||"").trim()}; if(email && await Model.exists({email})) return json({message:"This email is already registered to a student"},409); if(number && await Model.exists({number})) return json({message:"This phone number is already registered to a student"},409); if(Object.values(slot).every(Boolean)&&await Model.exists(slot))return json({message:`Roll ${slot.roll} is already occupied in Class ${slot.class}-${slot.section}`},409); Object.assign(item,slot,{email,number}); } }
