...
import { ArrowRight, BellRing, BookOpen, CalendarDays, Edit3, Eye, EyeOff, FileChartColumn, GraduationCap, KeyRound, LogOut, ReceiptIndianRupee, School, Sparkles, X } from "lucide-react";
...
export default function StudentDashboard() {
  ...
  const [student, setStudent] = useState(() => JSON.parse(localStorage.getItem("studentData") || "null"));
  ...
  const savePin = async () => {
    ...
    setSavingPin(false); if (error) return alert(error.message || "PIN update failed");
    const updated = data || { ...student, pin: pinValue }; setStudent(updated); localStorage.setItem("studentData", JSON.stringify(updated)); setEditingPin(false); setShowPin(true);
  };
