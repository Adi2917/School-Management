...
export default function StudentResult(){
  ...
  const rows=useMemo(()=>allResults.filter(row=>row.exam_type_id===selected),[allResults,selected]);const exam=exams.find(item=>item.id===selected);const total=rows.reduce((sum,row)=>sum+Number(row.obtained_marks??row.marks??0),0);const maximum=rows.reduce((sum,row)=>sum+Number(row.full_marks??row.max_marks??100),0);const percentage=maximum?total/maximum*100:0;
  ...
  return <main className="student-result-shell">
    ...
    {!exams.length?<section className="no-published-result"><BookOpenCheck/><h2>Result not available</h2><p>Your school has not published a result yet.</p></section>:<div className="student-result-layout">
      <aside className="student-exam-list"><small>PUBLISHED RESULTS</small><h2>Examinations</h2>{exams.map(item=>{const examRows=allResults.filter(row=>row.exam_type_id===item.id);const score=examRows.reduce((sum,row)=>sum+Number(row.obtained_marks??row.marks??0),0);const max=examRows.reduce((sum,row)=>sum+Number(row.full_marks??row.max_marks??100),0);return <button className={selected===item.id?"active":""} key={item.id} onClick={()=>setSelected(item.id)}><span><BookOpenCheck/></span><div><b>{item.name}</b><small>{examRows.length} subjects · {max?(score/max*100).toFixed(1):0}%</small></div></button>})}</aside>
      <section className="published-marksheet" id="printable-result">
        ...
        <div className="published-table"><div><b>Subject</b><b>Obtained</b><b>Full marks</b></div>{rows.map(row=><div key={row.id}><span>{row.subject}</span><strong>{row.obtained_marks??row.marks}</strong><span>{row.full_marks??row.max_marks??100}</span></div>)}</div>
