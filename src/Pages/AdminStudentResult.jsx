...
export default function AdminStudentResult() {
  ...
  const deleteExam = async (exam) => { if (!confirm(`Delete ${exam.name} and its saved results?`)) return; await supabase.from("results").delete().eq("exam_type_id", exam.id).eq("school_code", student.school_code); await supabase.from("exam_types").delete().eq("id", exam.id); if (selectedExam === exam.id) { setSelectedExam(""); setMarks({}); setMaxMarks({}); setResultExists(false); } fetchExamTypes(student.school_code); };
  const deleteResult = async () => { if (!selectedExam || !confirm("Delete this student's saved result?")) return; await supabase.from("results").delete().eq("student_id", studentId).eq("exam_type_id", selectedExam); setMarks({}); setMaxMarks({}); setResultExists(false); alert("Result deleted"); };
  ...
  return <div className="admin-result-container">
    ...
    {!!examTypes.length && <div className="saved-exams">{examTypes.map(exam => <span key={exam.id} className={selectedExam === exam.id ? "active" : ""}><button onClick={() => setSelectedExam(exam.id)}>{exam.name}</button><button className="exam-delete" onClick={() => deleteExam(exam)} aria-label={`Delete ${exam.name}`}>×</button></span>)}</div>}
    ...
    <button className="save-btn" onClick={saveResult}>{resultExists ? "Update Result" : "Save Result"}</button>{resultExists && <div className="result-actions"><button className="view-result-btn" onClick={() => navigate(`/StudentResult/${studentId}`)}>View Result</button><button className="delete-result-btn" onClick={deleteResult}>Delete Result</button></div>}
